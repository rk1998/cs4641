...
def svm_classifier(X, y, split_amount, plot=True, X_test=None, y_test=None, kernel_type='poly'):
	...
	if kernel_type == 'poly':
		...
		train_scores, test_scores = validation_curve(svm, X_train, y_train, param_name='degree'
		, param_range=poly_degree_range, cv=5, n_jobs=1)
		...
		best_degree = poly_degree_range[list(test_scores_mean).index(max(test_scores_mean))]

		svm.set_params(degree=best_degree)
		training_sizes = np.linspace(.1, 1.0, 5)
		train_sizes, train_scores_learn, test_scores_learn = learning_curve(svm, X_train, y_train,
		...
		if plot:
			...
			plt.figure()
			plt.grid()
			plt.title("SVM Validation Curve (Polynomial Kernel)")
			plt.plot(poly_degree_range, train_scores_mean, label='training_score', color='darkorange')
			plt.fill_between(poly_degree_range, train_scores_mean - train_scores_std,
			             train_scores_mean + train_scores_std, alpha=0.2,
			             color="darkorange", lw=lw)
			plt.plot(poly_degree_range, test_scores_mean, label='cross_validation_score', color='navy')
			plt.fill_between(poly_degree_range, test_scores_mean - test_scores_std,
			...
			title = "SVM Learning Curve (Polynomial Degree = " + str(best_degree) + " )"
			plt.figure(2)
			plt.grid()
			plt.title(title)
		...
		train_scores, test_scores = validation_curve(svm, X_train, y_train, param_name='gamma'
		, param_range=gamma_range, cv=5, n_jobs=1)
		...
		best_gamma = gamma_range[list(test_scores_mean).index(max(test_scores_mean))]
		svm.set_params(gamma=best_gamma)
		...
		if plot:
			...
			plt.figure()
			plt.grid()
			plt.title("SVM Validation Curve ( " + kernel_type + " kernel )")
			plt.plot(gamma_range, train_scores_mean, label='training_score', color='darkorange')
			plt.fill_between(gamma_range, train_scores_mean - train_scores_std,
             train_scores_mean + train_scores_std, alpha=0.2,
             color="darkorange", lw=lw)
			plt.plot(gamma_range, test_scores_mean, label='cross_validation_score', color='navy')
			...
			plt.xlabel("Gamma")
			plt.ylabel("Score")
			title = "SVM Learning Curve (Gamma = " + str(best_gamma) + " kernel =  "+ kernel_type + " )"
			plt.figure(2)
			plt.grid()
			plt.title(title)
			plt.fill_between(train_sizes, train_scores_learn_mean - train_scores_learn_std,
...
def measure_performance(X, y, svmclassifier):
	y_predict = svmclassifier.predict(X)
...
if sys.argv[1] == 'student':
	...
	split_amount = 0.3
	svm_classifier(X,y, split_amount, kernel_type=sys.argv[2])

	df_encoded = encode_features(df_string, df, reduce_classes=True)
	X,y = create_inputs(df_encoded, "G3")
	svm_classifier(X,y, split_amount, kernel_type=sys.argv[2])

if sys.argv[1] == 'exoplanet':
	...
	split_amount = 0.0
	svm_classifier(X_train, y_train, 0.0, kernel_type=sys.argv[2], X_test=X_test, y_test=y_test)
