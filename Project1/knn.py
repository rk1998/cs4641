...
def knneighbors(X, y, split_amount, plot=True, X_test=None, y_test=None):
	...
	best_k = neighbors_range[list(test_scores_mean).index(max(test_scores_mean))]
	knn.set_params(n_neighbors=best_k)

	training_sizes = np.linspace(.1, 1.0, 5)
	train_sizes, train_scores_learn, test_scores_learn = learning_curve(knn, X_train, y_train,
	...
	if plot:
		...
		plt.plot(neighbors_range, train_scores_mean, label='training_score', color='darkorange')
		plt.fill_between(neighbors_range, train_scores_mean - train_scores_std,
         train_scores_mean + train_scores_std, alpha=0.2,
         color="darkorange", lw=lw)
		plt.plot(neighbors_range, test_scores_mean, label='cross_validation_score', color='navy')
		plt.fill_between(neighbors_range, test_scores_mean - test_scores_std,
		...
		title = "kNN Learning Curve (k =" + str(best_k) + " )"
		plt.figure(2)
		plt.grid()
		plt.title(title)
...
def measure_performance(X, y, knn):
...
if sys.argv[1] == 'student':
	...
	split_amount = 0.0
	knneighbors(X_train, y_train, 0.0, X_test=X_test, y_test=y_test)
